...
import org.eib.cron.run.QueryFolderJob;
...
public class RunFolderView extends ViewPart {

	public static final String ID = "org.eib.cron.application.views.RunFolderView"; //$NON-NLS-1$
	private static Logger logger =Logger.getLogger("RunFolderView");	
	
	Scheduler scheduler;
	
	public RunFolderView() {
	}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {					
					for (int i=0; i< main1.get_querycron().length; i++){
						...
						JobDetail job = JobBuilder.newJob(QueryFolderJob.class)
						...
						job.getJobDataMap().put("_ftpID", main1.get_querycron()[i].get_ftpID());
						job.getJobDataMap().put("_mailID", main1.get_querycron()[i].get_mailID());
						job.getJobDataMap().put("_folderUrl", main1.get_querycron()[i].get_folderUrl());
						...
						logger.info("_ftpID= "+ main1.get_querycron()[i].get_ftpID());
						logger.info("_mailID= "+ main1.get_querycron()[i].get_mailID());
						logger.info("_folderUrl= "+ main1.get_querycron()[i].get_folderUrl());
