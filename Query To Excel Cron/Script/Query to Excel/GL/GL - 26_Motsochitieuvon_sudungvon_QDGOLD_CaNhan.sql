...
FROM
	(	SELECT  BRCD, BAL TDBAL, BAL1 TDBAL1, BAL2 TDBAL2, BAL3 TDBAL3, BAL4 TDBAL4, 0 TDBAL5, BAL6 TDBAL6
				...
				FROM
					(	SELECT	BRCD, F.CODE CODE, E.TDBAL TDBAL
                     		...
                     		(	SELECT	C.BRCD,SUM(DECODE('100','100',TDBAL100
								...
								FROM
									(	SELECT	G.TRDT, G.BRCD BRCD,
												G.ACCTCD ACCTCD,
												SUM ( decode ( G.CUSTTPCD, '100', decode(G.CCY, 'GD1',G.TDBAL, CS1.excrosscal('1000', 'VND',  &h_trdt, G.CCY, G.TDBAL, '01', 'GD1', '01'))
													        	,0)) TDBAL100,
												SUM ( decode ( G.CUSTTPCD,'100',0,decode(G.CCY, 'GD1',G.TDBAL, CS1.excrosscal('1000', 'VND',  &h_trdt, G.CCY, G.TDBAL, '01', 'GD1', '01'))
														       )) TDBAL200
									      FROM
						...
						WHERE	F.CAT_NAME='GLMISRPT'|| 'FU01' AND F.COUNTRY='VN'
							...
							AND		F.CODE IN('A01000','A05000','B06000','B06100','A02000','B06500')
					   )
				GROUP BY BRCD,CODE
			)
		...
		FROM	(	SELECT	A.BRCD,
						SUM(DECODE(A.CCY, 'GD1', A.LDRBAL,
									CS1.EXCROSSCAL ('1000', 'VND', &h_trdt, A.CCY, NVL(A.LDRBAL,0), '01', 'GD1', '01'))) TDBAL_VND,
						SUM(DECODE(A.CCY, 'GD1', DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.LDRBAL,0), 0),
									CS1.EXCROSSCAL ('1000', 'VND', &h_trdt, A.CCY, DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.LDRBAL,0), 0), '01', 'GD1', '01'))) DNTH_VND
				...
				SELECT	A.BRCD,
						SUM(DECODE(A.CCY, 'GD1', A.TDBAL,
									CS1.EXCROSSCAL ('1000', 'VND', &h_trdt, A.CCY, NVL(A.TDBAL,0), '01', 'GD1', '01'))) TDBAL_VND,
						SUM(DECODE(A.CCY, 'GD1', DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.TDBAL,0), 0),
									CS1.EXCROSSCAL ('1000', 'VND', &h_trdt, A.CCY, DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.TDBAL,0), 0), '01', 'GD1', '01'))) DNTH_VND
		...
		-- CHI TINH LOI NHUAN CHO ALL
 		UNION ALL

		SELECT	BRCD, 0 BAL, 0 BAL1, 0 BAL2, 0 BAL3, 0 BAL4, 0 BAL5, 0 BAL6
