			...
			FROM	(	SELECT  BRCD, BAL TDBAL, BAL1 TDBAL1, BAL2 TDBAL2, BAL3 TDBAL3, BAL4 TDBAL4, 0 TDBAL5
					FROM	(	SELECT	brcd,
							...
							FROM	 (	SELECT	brcd, F.CODE CODE, E.TDBAL TDBAL
									FROM	(	SELECT	c.brcd,SUM(decode('100','100',TDBAL100
				   ...
				   SELECT	BRCD, SUM(A.TDBAL) TDBAL, SUM(A.DNTH_VND) BAL1, 0 BAL2, 0  BAL3, 0  BAL4, 0  BAL5
				   FROM   (	SELECT	A.BRCD, SUM(A.LDRBAL) TDBAL, SUM(DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.LDRBAL,0), 0)) DNTH_VND
							...
							SELECT	E.BRCD, SUM(A.LDRBAL) TDBAL, SUM(DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.LDRBAL,0), 0)) DNTH_VND
							...
							SELECT	A.BRCD, SUM(A.TDBAL) TDBAL, SUM(DECODE(SUBSTR(A.ACCTCD, 4, 1), '1', NVL(A.TDBAL,0), 0)) DNTH_VND
					...
					SELECT	BRCD,0 BAL1, 0 BAL2  , 0 BAL3, 0 BAL4, 0 BAL5,decode('100','300',decode('VND','VND',ACRTDBAL,0),0) BAL6
