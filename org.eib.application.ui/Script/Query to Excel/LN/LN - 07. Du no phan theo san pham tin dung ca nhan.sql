...
/*HP20121106 - START*/
		...
		WHERE
			...
			AND A.DSBSBAL > 0
			AND EXISTS (SELECT CM.BRCD, CM.CUSTSEQ FROM CM1.TBCM_GENERAL CM WHERE CM.BRCD = A.BRCD AND CM.CUSTSEQ = A.CUSTSEQLN AND CM.CUSTTPCD LIKE '1%')
...
/*HP20121106 - END*/
		...
		WHERE
			...
			AND EXISTS (SELECT CM.BRCD, CM.CUSTSEQ FROM CM1.TBCM_GENERAL CM WHERE CM.BRCD = A.BRCD AND CM.CUSTSEQ = A.CUSTSEQ AND CM.CUSTTPCD LIKE '1%')
			-- AND LN1.CHK_LNCUSTTP(A.BRCD, '', '', A.CUSTSEQ, 'IND') = 'Y'
			AND LN1.CHKACCT20SBV(A.ACCTCD, A.CCY) = 'N'

		UNION ALL

/*HP20121106 - remove - START
		-- San vang 
		SELECT
			BRCD,
			'' DEPTCD,
...
HP20121106 - remove - END*/
		...
		(
			...
			WHERE
				...
				AND B.CCY = 'VND'
				AND EXISTS (SELECT CM.BRCD, CM.CUSTSEQ FROM CM1.TBCM_GENERAL CM WHERE CM.BRCD = B.BRCD AND CM.CUSTSEQ = B.CUSTSEQ AND CM.CUSTTPCD LIKE '1%')
				-- AND LN1.CHK_LNCUSTTP(B.BRCD, '', '', B.CUSTSEQ, 'IND') = 'Y'
		)
		WHERE
			LDRBAL > 0
