...
----								//AND LN1.CHKLNCUSTTYPE(MAST.BRCD, MAST.CUSTSEQ, DECODE('3', '1', 'ind', '2', 'cmp', '3', 'all')) = 'Y'
--								AND LN1.CHK_LNCUSTTP(MAST.BRCD,'','', MAST.CUSTSEQ, DECODE('3', '1', 'ind', '2', 'cmp', '3', 'all')) = 'Y'
--				)
...
--							AND MAS.BRCD = BR.BRCD
						SELECT
							...
							WHERE
								...
								AND MAS.BRCD = C.BRCD
								--AND LN1.CHKLNCUSTTYPE(MAST.BRCD, MAST.CUSTSEQ, DECODE('3', '1', 'ind', '2', 'cmp', '3', 'all')) = 'Y'
								AND LN1.CHK_LNCUSTTP(MAS.BRCD,'','', MAS.CUSTSEQ, DECODE('3', '1', 'ind', '2', 'cmp', '3', 'all')) = 'Y'
					)
			)
			GROUP BY	BRCD, deptcd, brnm--, CUSTTPCD, CUSTSEQ
			)D, --CHO_VAY
			(
				...
				(
					...
					--		--PGD
						...
						WHERE D.CARDTYPE = DECODE('3', '1','THEEIB', '3','THEEIB')
				...
				--							C.COUNT,	C.DEPTCD, DECODE(C.DEPTNM,'',C.LOCNM,C.LOCNM) DEPTNM
							...
							WHERE C.CARDTYPE = DECODE('3', '2','BUSINESS')
			...
			(
				...
				--TF
						...
						--		AND b.BRCD = c.BRCD
							...
							AND d.BRCD = '1000'
							AND d.CUSTTPCD LIKE DECODE('3', '1', '100', '%')
							AND d.CUSTTPCD NOT LIKE DECODE('3', '2', '100', '999')
						UNION ALL
						...
						WHERE
							...
							AND c.BRCD = '1000'
							AND c.CUSTTPCD LIKE DECODE('3', '1', '100', '%')
							AND c.CUSTTPCD NOT LIKE DECODE('3', '2', '100', '999')
				)A, CS1.TBCS_BRCD B
				WHERE
						A.BRCD = B.BRCD
				GROUP BY					A.BRCD, B.LCLBRNM
