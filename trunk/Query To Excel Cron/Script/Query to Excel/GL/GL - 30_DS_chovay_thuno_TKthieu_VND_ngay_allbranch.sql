-- theo ngay - VND
--define	h_trdt  = '20120131'


SELECT	a.BRCD, c.LCLBRSHRTNM, a.ACCTCD,
		DECODE(B.CUSTTPCD, '100', 'INV', 'CMP') CUSTTYPE,
		SUM(CS1.excrosscal('1000', 'VND', A.TRDT, A.CCY, NVL(A.CRAMT,0) , '01', 'VND', '01')) CRAMT,
		SUM(CS1.excrosscal('1000', 'VND', A.TRDT, A.CCY, NVL(A.DRAMT,0) , '01', 'VND', '01')) DRAMT,
...
FROM (
	...
	WHERE
		...
		AND A.TRDT BETWEEN &h_trdt AND &h_trdt
		AND A.DYHBR LIKE '%'
	) a, CM1.TBCM_GENERAL b, CS1.TBCS_BRCD c
WHERE
	A.BRCD = B.BRCD AND A.CUSTSEQ = B.CUSTSEQ
	AND a.BRCD = c.BRCD
	AND a.CCY     LIKE DECODE('4','1','%','2','%','3','GD%','4','VND')
	AND a.CCY NOT LIKE DECODE('4','1','AAA','2','GD%','3','AAA','4','AAA')
	AND a.CCY NOT LIKE DECODE('4','1','AAA','2','VND','3','AAA','4','AAA')
	AND B.CUSTTPCD LIKE '%'
