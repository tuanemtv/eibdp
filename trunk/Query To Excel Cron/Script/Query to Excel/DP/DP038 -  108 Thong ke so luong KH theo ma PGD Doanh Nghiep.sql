			...
			(
				...
				(
					...
					where
								...
								AND 	GEN.CUSTTPCD LIKE DECODE('2','1','100','%')
								AND 	GEN.CUSTTPCD NOT LIKE DECODE('2','1','999','2','100','999')
				--				AND	GEN.BRCD = C.BRCD
...
--							CUSTSEQ CIF_SL
				...
				--							AND mst.acctseQ = idx.ACCTSEQ
											...
											WHERE
												...
												AND MST.OPNCNCLFLG = '0'
												AND MST.OPNDT <= &h_trdt
												----AND ((MST.CLSFLG = '0') OR ((MST.CLSFLG = '2')AND(MST.CLSDT > &h_trdt)))
												and nvl(trim(MST.clsdt),'99999999')>&h_trdt
												AND GEN.CUSTTPCD LIKE DECODE('2','1','100','%')
												AND GEN.CUSTTPCD NOT LIKE DECODE('2','1','999','2','100','999')
												AND mst.BRCD = br.BRCD
...
----								//AND LN1.CHKLNCUSTTYPE(MAST.BRCD, MAST.CUSTSEQ, DECODE('2', '1', 'ind', '2', 'cmp', '1', 'all')) = 'Y'
...
--							AND MAS.BRCD = BR.BRCD
						SELECT
							...
							WHERE
								...
								AND LN1.CHK_LNCUSTTP(MAS.BRCD,'','', MAS.CUSTSEQ, DECODE('2', '1', 'ind', '2', 'cmp', '3', 'all')) = 'Y'
					)
			)
			GROUP BY	BRCD, deptcd, brnm--, CUSTTPCD, CUSTSEQ
			)D, --CHO_VAY
			(
				...
				(
					...
					--		--PGD
						...
						WHERE D.CARDTYPE = DECODE('2', '1','THEEIB', '3','THEEIB')
				...
				--							C.COUNT,	C.DEPTCD, DECODE(C.DEPTNM,'',C.LOCNM,C.LOCNM) DEPTNM
							...
							WHERE C.CARDTYPE = DECODE('2', '2','BUSINESS')
			...
			(
				...
				--TF
						...
						--		AND b.BRCD = c.BRCD
							...
							AND d.BRCD = '1000'
							AND d.CUSTTPCD LIKE DECODE('2', '1', '100', '%')
							AND d.CUSTTPCD NOT LIKE DECODE('2', '2', '100', '999')
						UNION ALL
						...
						WHERE
							...
							AND c.BRCD = '1000'
							AND c.CUSTTPCD LIKE DECODE('2', '1', '100', '%')
							AND c.CUSTTPCD NOT LIKE DECODE('2', '2', '100', '999')
				)A, CS1.TBCS_BRCD B
				WHERE
						A.BRCD = B.BRCD
				GROUP BY					A.BRCD, B.LCLBRNM
